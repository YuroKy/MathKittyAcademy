...
import { snapshotExerciseAnswer } from '@/domain/exercises/validateAnswer'
...
class SessionRepository {
  ...
  async findActiveSession(profileId: string): Promise<LearningSession | undefined> {
    return (
      await db.sessions
  ...
  async saveAnswerDraft(
    ...
    await db.sessions.update(sessionId, {
      answerDrafts: {
        ...
        [exerciseId]: snapshotExerciseAnswer(answer),
      },
    })
  }
  ...
  async recordAttempt(input: RecordAttemptInput): Promise<ExerciseAttempt> {
    const existingAttempts = await db.attempts
      ...
      .filter(
        (entry) =>
          ...
          JSON.stringify(entry.normalizedAnswer) === JSON.stringify(input.normalizedAnswer) &&
    ...
    const errorType = input.isCorrect
      ? undefined
      : classifyExerciseError(
          typeof input.normalizedAnswer === 'string' ? input.normalizedAnswer : '',
